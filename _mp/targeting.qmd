...
source("../utils.R")
source("../party_utils_mp.R")


mp <- openxlsx::read.xlsx("../data/India Spend Reports - 90 days to Oct 23rd 2023.xlsx", sheet = 3) %>% 
  janitor::clean_names() %>% 
  filter(done)


election_dat30 <- election_dat30 %>% 
  filter(internal_id %in% mp$page_id)
...
How much did the campaigns of political parties spend on different target criteria? The charts below show both the total and the expenditures per party.
